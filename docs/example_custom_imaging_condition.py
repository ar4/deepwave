...
source_locations[:, 0, 1] = torch.linspace(0,
                                           nx - 1,
                                           n_shots,
                                           dtype=torch.long,
                                           device=device)
receiver_locations = torch.zeros(n_shots,
                                 n_receivers_per_shot,
                                 2,
                                 dtype=torch.long,
                                 device=device)
receiver_locations[..., 0] = acquisition_depth
receiver_locations[0, :, 1] = torch.linspace(0,
                                             nx - 1,
                                             n_receivers_per_shot,
                                             dtype=torch.long,
                                             device=device)
...
source_amplitudes = (deepwave.wavelets.ricker(freq, nt, dt,
                                              peak_time).repeat(n_shots, 1,
                                                                1).to(device))

# Make data with true data
d_true = deepwave.scalar(v_true,
                         dx,
                         dt,
...
def method_1():
    ...
    source_amplitudes_up = deepwave.common.upsample(source_amplitudes,
                                                    step_ratio)
    ...
    wavefield_0 = torch.zeros(*wavefield_size,
                              device=device,
                              requires_grad=True)
    ...
    for i, chunk in enumerate(
            torch.chunk(source_amplitudes_up, n_segments, dim=-1)):
        ...
        if chunk.shape[-1] > 1:
            ...
            (wavefield_0, wavefield_m1, psiy_m1, psix_m1,
             zetay_m1, zetax_m1, receiver_amplitudes_chunk) = \
                wrap(chunk[...,:-1], wavefield_0,
                     wavefield_m1,
                     psiy_m1, psix_m1,
                     zetay_m1, zetax_m1)
            receiver_amplitudes[..., k:k + chunk.shape[-1] -
                                1] = receiver_amplitudes_chunk

        # We then save the source (forward) wavefield (part 1)
        source_wavefields[i] = (wavefield_m1.detach()[:, pml_width:-pml_width,
                                                      pml_width:-pml_width])
        ...
        (wavefield_0, wavefield_m1, psiy_m1, psix_m1,
         zetay_m1, zetax_m1, receiver_amplitudes_chunk) = \
            wrap(chunk[...,-1:], wavefield_0,
                 ...
                 psiy_m1, psix_m1,
                 zetay_m1, zetax_m1)
        receiver_amplitudes[..., k + chunk.shape[-1] - 1:k +
                            chunk.shape[-1]] = receiver_amplitudes_chunk
        ...
        source_wavefields[i] += (
            ...
            - 2 * wavefield_m1.detach()[:, pml_width:-pml_width,
                                        pml_width:-pml_width])
    ...
    def backward(ctx, wfc, wfp, psiy, psix, zetay, zetax, grad_r):
        ...
        for i in range(nt // step_ratio - 1, -1, -1):
            ...
            if v.is_cuda:
                ...
                chunk = grad_r[:, i * step_ratio:(i + 1) *
                               step_ratio].contiguous()
            backward(v2dt2.data_ptr(), chunk.data_ptr(), wfc.data_ptr(),
                     wfp.data_ptr(), psiy.data_ptr(), psix.data_ptr(),
                     psiyn.data_ptr(), psixn.data_ptr(), zetay.data_ptr(),
        ...
        s = (slice(None), slice(fd_pad, -fd_pad), slice(fd_pad, -fd_pad))
        return grad_v, None, wfc[s], -wfp[s], psiy[s], psix[s], zetay[s], zetax[
            s], None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None
...
def method_2():
    ...
    receiver_amplitudes = method2_scalar(v,
                                         dx,
                                         dt,
                                         source_amplitudes=source_amplitudes,
                                         source_locations=source_locations,
...
vmax = 0.0003
im = ax[0].imshow(method_1_grad.cpu()[1:-1, 1:-1],
                  aspect='auto',
                  cmap='gray',
                  vmin=-vmax,
                  vmax=vmax)
...
im = ax[1].imshow(method_2_grad.cpu()[1:-1, 1:-1],
                  aspect='auto',
                  cmap='gray',
                  vmin=-vmax,
                  vmax=vmax)
...
im = ax[2].imshow(regular_deepwave_grad.cpu()[1:-1, 1:-1],
                  aspect='auto',
                  cmap='gray',
                  vmin=-vmax,
                  vmax=vmax)
