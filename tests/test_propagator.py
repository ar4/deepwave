...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pml_width", [1, 2, 3], "Expected pml_width to be of length 1 or 4, got 3.", RuntimeError),
        ("pml_freq", -1.0, "pml_freq must be non-negative.", ValueError),
        ("max_vel", -1.0, "max_vel is less than the actual maximum velocity.", UserWarning), # This will be a warning, not an error
        ("model_gradient_sampling_interval", -1, "model_gradient_sampling_interval must be >= 0", ValueError),
...
def test_scalar_forward_invalid_optional_args(prop, arg_name, arg_value, expected_error_match, expected_error_type):
    source_amplitudes = torch.zeros(1, 1, 2)
    source_locations = torch.zeros(1, 1, 2, dtype=torch.long)
    receiver_locations = torch.zeros(1, 1, 2, dtype=torch.long)
    dt = 0.004
...
@pytest.mark.parametrize(
    ...
    [
        ((1, 1), (1, 1, 2), (1, 1, 2), "source amplitudes Tensors should have 3 dimensions"), # wrong num dims
        ((1, 1, 2), (2, 1, 2), (1, 1, 2), "Expected source amplitudes to have size 2 in the batch dimension"), # shape mismatch 1
        ((1, 1, 2), (1, 2, 2), (1, 1, 2), "Expected source amplitudes and locations to be the same size in the n_sources_per_shot dimension"), # shape mismatch 2
        ((1, 1, 2), (1, 1, 3), (1, 1, 2), "Source locations must have 2 dimensional coordinates, but found one with 3."), # shape mismatch 3
        ((2, 1, 1), (1, 1, 3), (1, 1, 2), "Expected source amplitudes to have size 1 in the batch dimension"), # shape mismatch 4
    ]
)
def test_input_shape_mismatch(prop, source_amplitudes_shape, source_locations_shape, receiver_locations_shape, expected_error_match):
...
@pytest.mark.parametrize(
    ...
    [
        ((3, NZ + 2 * 20, NY + 2 * 20), "Inconsistent batch size"), # batch size too big
        ((1, NZ + 2 * 20, NY + 2 * 20), "Inconsistent batch size"), # batch size too small
        ((2, NZ + 2 * 20 + 1, NY + 2 * 20), "Survey extents are larger than the model."), # too big in Z
        ((2, NZ + 2 * 20, NY + 2 * 20 + 1), "Survey extents are larger than the model."), # too big in Y
    ]
)
def test_wavefield_shape_mismatch(prop, wavefield_shape, expected_error_match):
    ...
    with pytest.raises(RuntimeError, match=expected_error_match):
        prop(dt, source_amplitudes, source_locations, receiver_locations, wavefield_0=wavefield_0)
...
def test_pml_width_list_too_long(prop):
    ...
    receiver_locations = torch.zeros(2, 1, 2, dtype=torch.long)
    dt = 0.004
    with pytest.raises(RuntimeError, match="Expected pml_width to be of length 1 or 4, got 5."):
        prop(dt,
             source_amplitudes,
             source_locations,
             receiver_locations,
