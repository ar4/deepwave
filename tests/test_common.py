import pytest
import torch
from deepwave.common import _as_list, get_n_batch, set_grid_spacing, set_accuracy, set_pml_width, set_pml_freq, set_max_vel, set_nt, set_model_gradient_sampling_interval, set_freq_taper_frac, set_time_pad_frac, check_source_amplitudes_locations_match, set_source_amplitudes, ResampleConfig, set_receiver_amplitudes, check_points_per_wavelength, cosine_taper_end, zero_last_element_of_final_dimension, upsample, downsample, cfl_condition_n
import re
import math
...
def test_as_list_invalid_type():
    with pytest.raises(TypeError, match=re.escape("test_param must be a float, int, torch.Tensor, or a sequence of floats/ints, got <class 'str'>.")):
        _as_list("invalid", "test_param", int)

def test_as_list_list_invalid_element_type():
    with pytest.raises(TypeError, match=re.escape("Elements in test_param could not be converted to int. Original error: invalid literal for int() with base 10: 'invalid'")):
...
def test_set_grid_spacing_list_incorrect_length():
    with pytest.raises(RuntimeError, match=re.escape("grid_spacing must have 1 or 2 elements, got 3.")):
        set_grid_spacing([10.0, 11.0, 12.0], 2)
    with pytest.raises(RuntimeError, match=re.escape("grid_spacing must have 1 or 3 elements, got 2.")):
...
def test_set_accuracy_invalid_value():
    with pytest.raises(ValueError, match=re.escape("accuracy must be 2, 4, 6, or 8, got 3")):
        set_accuracy(3)
    with pytest.raises(ValueError, match=re.escape("accuracy must be 2, 4, 6, or 8, got 0")):
        set_accuracy(0)
    with pytest.raises(ValueError, match=re.escape("accuracy must be 2, 4, 6, or 8, got 10")):
        set_accuracy(10)
...
def test_set_pml_width_list_incorrect_length():
    with pytest.raises(RuntimeError, match=re.escape("Expected pml_width to be of length 1 or 4, got 3.")):
        set_pml_width([10, 11, 12], 2)
    with pytest.raises(RuntimeError, match=re.escape("Expected pml_width to be of length 1 or 6, got 5.")):
        set_pml_width([1, 2, 3, 4, 5], 3)

def test_set_pml_width_invalid_type():
    with pytest.raises(TypeError, match=re.escape("pml_width must be a float, int, torch.Tensor, or a sequence of floats/ints, got <class 'str'>.")):
...
def test_set_pml_freq_none():
    ...
    with pytest.warns(UserWarning, match=re.escape("pml_freq was not set, so defaulting to 25.0.")):
...
def test_set_pml_freq_above_nyquist():
    ...
    with pytest.warns(UserWarning, match=re.escape("pml_freq 150.0 is greater than the Nyquist frequency 125.0.")):
...
def test_set_max_vel_less_than_actual():
    with pytest.warns(UserWarning, match=re.escape("max_vel is less than the actual maximum velocity.")):
...
def test_set_nt_none_without_source_amplitudes():
    ...
    step_ratio = 1
    with pytest.raises(RuntimeError, match=re.escape("nt or source amplitudes must be specified")):
...
def test_set_nt_with_both_inconsistent():
    source_amplitudes = [torch.zeros(1, 1, 100)]
    step_ratio = 1
    with pytest.raises(RuntimeError, match=re.escape("Only one of nt or source amplitudes should be specified")):
...
def test_set_model_gradient_sampling_interval_negative_value():
    with pytest.raises(ValueError, match=re.escape("model_gradient_sampling_interval must be >= 0")):
...
def test_set_freq_taper_frac_out_of_range():
    with pytest.raises(ValueError, match=re.escape("freq_taper_frac must be in [0, 1], got -0.1")):
        set_freq_taper_frac(-0.1)
    with pytest.raises(ValueError, match=re.escape("freq_taper_frac must be in [0, 1], got 1.1")):
        set_freq_taper_frac(1.1)

def test_set_freq_taper_frac_invalid_type():
    with pytest.raises(TypeError, match=re.escape("freq_taper_frac must be a float or an int.")):
...
def test_set_time_pad_frac_out_of_range():
    with pytest.raises(ValueError, match=re.escape("time_pad_frac must be in [0, 1], got -0.1")):
        set_time_pad_frac(-0.1)
    with pytest.raises(ValueError, match=re.escape("time_pad_frac must be in [0, 1], got 1.1")):
        set_time_pad_frac(1.1)

def test_set_time_pad_frac_invalid_type():
    with pytest.raises(TypeError, match=re.escape("time_pad_frac must be a float or an int.")):
...
def test_check_source_amplitudes_locations_match_length_mismatch():
    ...
    source_locations = [torch.zeros(1, 1, 2), torch.zeros(1, 2, 2)]
    with pytest.raises(RuntimeError, match=re.escape("The same number of source_amplitudes (1) and source_locations (2) must be provided.")):
        check_source_amplitudes_locations_match(source_amplitudes, source_locations)

def test_check_source_amplitudes_locations_match_none_mismatch():
    source_amplitudes = [torch.zeros(1, 1, 10), None]
    source_locations = [torch.zeros(1, 1, 2), torch.zeros(1, 2, 2)]
    with pytest.raises(RuntimeError, match=re.escape("Each pair of source locations and amplitudes must both be None or both be non-None.")):
        check_source_amplitudes_locations_match(source_amplitudes, source_locations)
...
def test_set_source_amplitudes_none():
    ...
    dtype = torch.float32
    result = set_source_amplitudes(source_amplitudes, n_batch, nt, resample_config, device, dtype)
    assert len(result) == 1
...
def test_set_source_amplitudes_invalid_ndim():
    ...
    device = torch.device("cpu")
    dtype = torch.float32
    with pytest.raises(RuntimeError, match=re.escape("source amplitudes Tensors should have 3 dimensions, but found one with 2.")):
...
def test_set_source_amplitudes_inconsistent_device():
    source_amplitudes = [torch.randn(2, 3, 100, device="cuda") if torch.cuda.is_available() else torch.randn(2, 3, 100)]
    n_batch = 2
    nt = 100
    resample_config = ResampleConfig()
    device = torch.device("cpu")
    ...
    if torch.cuda.is_available():
        with pytest.raises(RuntimeError, match=re.escape(f"Inconsistent device: Expected all Tensors be on device cpu, but found a source amplitudes Tensor on device cuda.")):
            set_source_amplitudes(source_amplitudes, n_batch, nt, resample_config, device, dtype)
    else:
...
def test_set_source_amplitudes_inconsistent_dtype():
    ...
    device = torch.device("cpu")
    dtype = torch.float32
    with pytest.raises(RuntimeError, match=re.escape(f"Inconsistent dtype: Expected source amplitudes to have datatype {dtype}, but found one with dtype {torch.float64}.")):
        set_source_amplitudes(source_amplitudes, n_batch, nt, resample_config, device, dtype)
...
def test_set_source_amplitudes_inconsistent_batch_size():
    ...
    device = torch.device("cpu")
    dtype = torch.float32
    with pytest.raises(RuntimeError, match=re.escape(f"Expected source amplitudes to have size {n_batch} in the batch dimension, but found one with size 3.")):
        set_source_amplitudes(source_amplitudes, n_batch, nt, resample_config, device, dtype)

def test_set_source_amplitudes_inconsistent_nt():
    source_amplitudes = [torch.randn(2, 3, 40)] # nt is 100, step_ratio is 1, so expected time samples is 100
    n_batch = 2
    nt = 100
    resample_config = ResampleConfig(step_ratio=1)
    device = torch.device("cpu")
    dtype = torch.float32
    with pytest.raises(RuntimeError, match=re.escape(f"Inconsistent number of time samples: Expected source amplitudes to have {nt // resample_config.step_ratio} time samples, but found one with 40.")):
        set_source_amplitudes(source_amplitudes, n_batch, nt, resample_config, device, dtype)
...
def test_set_receiver_amplitudes_valid():
    ...
    assert len(result) == 2
    assert result[0].shape == (nt, n_batch, 3)
    assert result[1].shape == (nt, n_batch, 4)
    assert result[0].device == device
    assert result[1].device == device
    assert result[0].dtype == dtype
    assert result[1].dtype == dtype

def test_set_receiver_amplitudes_empty_list():
    receiver_locations = []
    n_batch = 2
    nt = 100
    device = torch.device("cpu")
...
def test_set_receiver_amplitudes_no_receivers_per_shot():
    receiver_locations = [torch.zeros(2, 0, 2)]
    n_batch = 2
    nt = 100
    device = torch.device("cpu")
    dtype = torch.float32
...
def test_check_points_per_wavelength_warns():
    ...
    with pytest.warns(UserWarning, match=re.escape("At least six grid cells per wavelength is recommended, but at a frequency of 25.0, a minimum non-zero velocity of 100.0, and a grid cell spacing of 5.0, there are only 0.80.")):
        check_points_per_wavelength(min_nonzero_vel, pml_freq, grid_spacing)
...
def test_cosine_taper_end_basic():
    ...
    n_taper = 5
    expected = torch.tensor([1., 1., 1., 1., 1., 0.9045, 0.6545, 0.3455, 0.0955, 0.])
...
def test_cosine_taper_end_full_taper():
    signal = torch.ones(5)
    n_taper = 5
    expected = torch.tensor([0.9045, 0.6545, 0.3455, 0.0955, 0.])
    result = cosine_taper_end(signal, n_taper)
    assert torch.allclose(result, expected, atol=1e-4)
...
def test_cfl_condition_n_zero_max_vel():
    ...
    dt = 0.004
    max_vel = 0.0
    with pytest.raises(RuntimeError, match=re.escape("max_abs_vel must be greater than zero.")):
...
def test_cfl_condition_n_invalid_type_grid_spacing():
    with pytest.raises(TypeError, match=re.escape("grid_spacing must be a sequence of floats or ints.")):
