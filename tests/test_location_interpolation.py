...
def test_get_hicks_for_one_location_dim_invalid_halfwidth():
    with pytest.raises(RuntimeError, match="halfwidth must be in \[1, 10\]"):
        _get_hicks_for_one_location_dim({}, 0.5, 0, torch.tensor(1.0), [False, False], [-0.5, 19.5], 20)
    with pytest.raises(RuntimeError, match="halfwidth must be in \[1, 10\]"):
        _get_hicks_for_one_location_dim({}, 0.5, 11, torch.tensor(1.0), [False, False], [-0.5, 19.5], 20)

def test_get_hicks_for_one_location_dim_invalid_beta():
    with pytest.raises(RuntimeError, match="beta must be non-negative."):
        _get_hicks_for_one_location_dim({}, 0.5, 4, torch.tensor(-1.0), [False, False], [-0.5, 19.5], 20)
...
def test_get_hicks_for_one_location_dim_invalid_extent():
    with pytest.raises(RuntimeError, match="extent must be a list of two floats."):
        _get_hicks_for_one_location_dim({}, 0.5, 4, torch.tensor(1.0), [False, False], [-0.5], 20)
    with pytest.raises(RuntimeError, match="extent must be a list of two floats."):
        _get_hicks_for_one_location_dim({}, 0.5, 4, torch.tensor(1.0), [False, False], [-0.5, 19.5, 20.0], 20)
    with pytest.raises(RuntimeError, match="extent must be a list of two floats."):
        _get_hicks_for_one_location_dim({}, 0.5, 4, torch.tensor(1.0), [False, False], ["a", 19.5], 20)
...
def test_get_hicks_for_one_location_dim_invalid_n_grid_points():
    with pytest.raises(RuntimeError, match="n_grid_points must be positive."):
        _get_hicks_for_one_location_dim({}, 0.5, 4, torch.tensor(1.0), [False, False], [-0.5, 19.5], 0)
    with pytest.raises(RuntimeError, match="n_grid_points must be positive."):
        _get_hicks_for_one_location_dim({}, 0.5, 4, torch.tensor(1.0), [False, False], [-0.5, 19.5], -1)
