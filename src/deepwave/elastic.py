...
from deepwave.elastic_equation import ElasticEquation, zero_edges_and_interiors
...
def elastic_func(
    python_backend: Union[Literal["eager", "jit", "compile"], bool],
    pml_profiles: List[torch.Tensor],
    grid_spacing: List[float],
    dt: float,
    nt: int,
    ...
    *args: torch.Tensor,
    ...
    # Args contains: models, source_amplitudes, sources_i, receivers_i, wavefields
    # We need to insert pml_profiles after source_amplitudes
    ...
    # args is a tuple, so we can construct new args
    split_point = num_models + num_source_amplitudes
    new_args = args[:split_point] + tuple(pml_profiles) + args[split_point:]
