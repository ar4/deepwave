...
def scalar(v: Tensor,
    ...
    wavefields = [None if wf.numel() == 0 else wf for wf in wavefields]
    v = models[0]
...
def scalar_prop(v: Tensor, source_amplitudes: Optional[Tensor],
    ...
    for t in range(0, nt, step_ratio):
        wfc, wfp, psiy, psix, zetay, zetax = forward_step_ratio(
            wfc, wfp, psiy, psix, zetay, zetax, v2dt2, ay, ax, by, bx,
            daydy, daxdx, dbydy, dbxdx, fd_coeffs1y, fd_coeffs2y,
            fd_coeffs1x, fd_coeffs2x, accuracy, step_ratio,
...
def forward_kernel(wfc: Tensor, wfp: Tensor,
                   ...
                   v2dt2: Tensor, ay: Tensor, ax: Tensor,
                   by: Tensor, bx: Tensor,
...
def forward_step_ratio(
        ...
        for i in range(step_ratio):
            ...
            if source_amplitudes is not None and sources_i is not None:
                add_sources(wfn, source_amplitudes[i], sources_i)
            if receivers_i is not None:
