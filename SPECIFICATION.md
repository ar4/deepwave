...
Everyone who looks at the code should be impressed by how clean, clear, and Pythonic it is. It should be an exemplar of best practices. It is, however, a high performance code implementing computationally intensive operations that will be limited by available computational power and memory. Complicated C and CUDA code implementing the inner loops of these calculations might therefore need to be tolerated to ensure maximum performance.

All of the code must be comprehensively tested, so the test suite is a crucial component of the package. Like the main code, it should be clear, well-structured, and follow best practices. It should also be extremely thorough, verifying the correctness of the code (including extensively testing that it obeys physics), checking the behaviour of the code for edge cases, and ensuring that invalid arguments are caught and helpful error messages are provided to users. Internal components should also be unit tested so that developers can more confidently make changes.

To help users and developers avoid mistakes, the code should use type hinting. As many users will call the code from a Notebook environment, these type hints might not be sufficient to avoid passing incorrect inputs, so exhaustive input checking with helpful error messages is also needed to quickly catch mistakes. As it is a scientific code that will be used by researchers to test ideas, there should not be unnecessary restrictions on inputs. For example, if the code needs a list of floating point numbers for a parameter, the user should be able to pass in a Python List or Tuple, a PyTorch Tensor or NumPy Array with the appropriate number of elements, or any other type that can reasonably and unambiguously provide the necessary information. Even though floating point numbers are needed, passing integers is acceptable as they can be reasonably and unambiguously converted, but complex numbers should probably not be accepted as converting them is not reasonable and they indicate that the user made a mistake. There may be cases where a warning is more appropriate than an error, such as when an input is valid but it is unusual and so might not be what the user intended. The goal is to help users to quickly catch mistakes.
