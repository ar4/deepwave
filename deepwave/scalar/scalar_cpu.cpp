...
inline ptrdiff_t location_index(const ptrdiff_t *__restrict__ const arr,
...
void add_sources(TYPE *__restrict__ const next_wavefield,
  ...
  for (ptrdiff_t shot = 0; shot < num_shots; shot++) {
    for (ptrdiff_t source = 0; source < num_sources_per_shot; source++) {
      ...
      ptrdiff_t i = location_index(source_locations, shape, s);
      ptrdiff_t si = shot * shape[0] * shape[1] * shape[2] + i;
